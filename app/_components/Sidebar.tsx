...
// Se o seu handleLogout não estiver funcionando, comente a linha abaixo para testar
...
export default function Sidebar() {
  ...
  return (
    <>
      {/* 1. BOTÃO HAMBÚRGUER (Aparece só no Celular) */}
      <button 
        onClick={toggleMobile}
        className="md:hidden fixed top-8 left-8 z-[60] bg-sky-500 text-white p-3 rounded-2xl shadow-lg active:scale-90 transition-transform"
      >
        {isOpenMobile ? <X size={24} /> : <Menu size={24} />}
      </button>

      {/* 2. OVERLAY (Fundo escuro no celular) */}
      {isOpenMobile && (
        <div 
      ...
      <aside 
        ...
        `}
      >
        {/* Botão da Setinha (Aparece só no Desktop) */}
        <button 
        ...
        <div className="mt-auto pt-6 border-t border-slate-700/50 space-y-2">
          ...
          >
            ...
            {(isExpanded || isOpenMobile) && <span className="text-sm">Sair</span>}
          </button>
        </div>
      </aside>
    </>
...
function SidebarItem({ icon, label, href, active, isExpanded, isOpenMobile }: any) {
  ...
  return (
    ...
    >
      <span className={`${active ? "text-white" : "text-sky-400"} shrink-0`}>{icon}</span>
